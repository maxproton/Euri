...
if __name__ == '__main__':
    ...
    if args.page:
        if args.page_file != None:
            with open(args.page_file, 'r') as file:
                data = {}

                for index, url in enumerate(file, start=0):
                    if args.domain in url:
                        url = url.rstrip('\n')

                        # Header extractor
                        headers[url] = module_headers.get_header(url)
                        for key, header in headers[url].items():
                            email_addresses = email_addresses + module_email.extract_emails(header)

                        # Enumerate cookies
                        cookies[url] = module_headers.get_cookies(headers[url].items())

                        #check for technology stack
                        for key, header in headers[url].items():
                            if key != "Set-Cookie" and key != "Cookie":
                                server[url] = module_framework.check_for_server_hinting(header)

                        technology_stack[url]['server'] = server[url]
                        cookie_stack = []
                        for key, value in cookies[url].items():
                            cookie_stack = cookie_stack + module_framework.check_for_cookie_hinting(key)
                        technology_stack[url]['language'] = cookie_stack

                        # Content Analysis starts
                        content = module_content_analysis.get_content(url)
                        links[url] = module_content_analysis.link_harvest(url, content, args.verbose)
                        link_images = {}
                        for key, values in links[url].items():
                            if key == "live":
                                for value in values:
                                    if module_content_analysis.check_link_for_image(value):
                                        link_images[url][value] = value
                        # Images
                        images[url] = module_content_analysis.extract_image_metadata(content, url, link_images[url],
                                                                                args.verbose)
                        # social media links
                        social[url] = module_content_analysis.find_social_media(links[url])

                        # git references
                        repo[url] = module_content_analysis.find_github_references(links[url])

                        # email references
                        email_addresses = email_addresses + module_email.extract_emails(
                            module_content_analysis.remove_html(content))
                        ...
                        # ip extraction
                        ip_addresses.append(module_content_analysis.extract_ip_addresses(content))

                        # comments extractor
                        comments.append(module_content_analysis.extract_comments_from_html(content))
                        data[url] = {
                            'headers': headers[url],
                            'cookies': cookies[url],
                            'email_addresses': email_addresses,
                            'technology_stack': technology_stack[url],
                            'images': images[url],
                            'links': links[url],
                            'social': social[url],
                            'repo': repo[url],
            ...
            print(f"Page file required use the --page-file argument and provide full path")
            exit(1)
